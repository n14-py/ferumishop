...
async function generateContentWithRetry(prompt, retries = 0) {
    ...
    try {
        ...
        try {
            ...
            if (retries >= cfg.keys.length) {
                console.error('[Gemma] Todas las claves agotadas. Esperando 60s antes de fallar...');
                await sleep(60 * 1000);
            }
            throw err;
        }
...
function extractJsonObject(raw) {
    ...
    try {
        ...
        throw new Error(`JSON de la IA inválido: ${err.message}`);
    }
}

async function generateJsonObjectWithRetry(prompt, { attempts = 5 } = {}) {
    let lastError;
    for (let i = 0; i < attempts; i += 1) {
        try {
            ...
            lastError = err;
            console.warn(`[Gemma] JSON intento ${i + 1}/${attempts}: ${String(err.message || err).slice(0, 160)}`);
            rotateKey();
            await sleep(1500 * (i + 1));
        }
    }
    throw lastError || new Error('La IA no pudo armar un JSON válido.');
}

module.exports = {
    config,
    generateContentWithRetry,
    generateJsonObjectWithRetry,
    extractJsonObject,
    rotateKey,
    sleep
};
