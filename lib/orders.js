...
const DEFAULT_STORE = {
    ...
    address: 'Ferumishop, Capiatá, Ruta 1 Ex km 21 - Paraguay',
    motoboltMaxKm: 30
};
...
const TRACKING_META = {
    ...
    pagado: { title: 'Pagado', detail: 'Recibimos tu pago seguro. Procesamos y preparamos tu pedido; te escribimos por WhatsApp a la brevedad.', icon: 'fa-check-circle', tone: 'ok' },
    preparando: { title: 'Preparando', detail: 'Estamos armando tu pedido. Si elegiste Moto Bolt Envíos, te contactamos en menos de 1 hora (de 9 a 20 h) para coordinar la entrega.', icon: 'fa-box-open', tone: 'info' },
    preparado: { title: 'Preparado', detail: 'Tu pedido ya está listo.', icon: 'fa-clipboard-check', tone: 'ok' },
    esperando_motobolt: { title: 'Esperando Motobolt', detail: 'Te escribimos para coordinar la entrega con Moto Bolt Envíos. Pedí el moto cuando te indiquemos; te pasamos la ubicación de Ferumishop.', icon: 'fa-motorcycle', tone: 'info' },
    enviando: { title: 'En camino', detail: 'Tu encomienda va con Transportadora TSI, Jaraha o la de tu preferencia hacia el interior.', icon: 'fa-truck', tone: 'info' },
...
function storeFromConfig(siteConfig) {
    const configuredKm = Number(siteConfig?.motoboltMaxKm);
    return {
        ...
        motoboltMaxKm: configuredKm === 40 ? 30 : (configuredKm || DEFAULT_STORE.motoboltMaxKm)
    };
}
...
function shippingOptions({ lat, lng, city, siteConfig }) {
    ...
    const maxKm = store.motoboltMaxKm;

    if (central && !far) {
        options.push({
            id: 'motobolt',
            icon: 'fa-motorcycle',
            label: 'Moto Bolt Envíos (Asunción y alrededores)',
            easyTitle: 'Moto Bolt Envíos',
            easyText: `Hasta ${maxKm} km. Pagás con Pagopar y, en menos de 1 hora, preparamos tu pedido y te escribimos al WhatsApp para coordinar la entrega.`,
            cost: 0,
            eta: 'Preparación y contacto en menos de 1 hora (9 a 20 h)',
            detail: `El local está en Capiatá, Ruta 1 Ex km 21. Cubrimos Asunción y alrededores (hasta ${maxKm} km). Pagopar es un pago seguro. Recibido el pedido, lo procesamos, lo preparamos y te contactamos en menos de 1 hora para concretar la entrega con Motobolt. El número de Pagopar debe ser tu WhatsApp. Horario de 9:00 a 20:00; si el pago entra fuera de ese horario, la coordinación pasa al día siguiente. Vos pedís el Motobolt cuando te avisemos; el costo del moto lo abonás al conductor.`
        });
    }
    ...
    if (far || !central) {
        options.push({
            id: 'transportadora',
            icon: 'fa-truck',
            label: 'Encomienda por transportadora',
            easyTitle: 'Encomienda por transportadora',
            easyText: 'Enviamos con Transportadora TSI, Transportadora Jaraha o la de tu preferencia. Te escribimos para coordinar agencia y retiro.',
            cost: 0,
            eta: 'Coordinamos agencia después del pago',
            detail: 'Motobolt no llega a esa distancia. Mandamos tu pedido por encomienda: Transportadora TSI, Transportadora Jaraha o la que vos prefieras. Después del pago te contactamos por WhatsApp para confirmar agencia y datos de retiro.'
        });
    } else {
        options.push({
            id: 'transportadora',
            ...
            label: 'Encomienda por transportadora',
            easyTitle: 'Encomienda por transportadora',
            easyText: 'Si preferís encomienda, enviamos con TSI, Jaraha o la transportadora que elijas. Coordinamos agencia por WhatsApp.',
            cost: 0,
            eta: 'Opción extra si no querés Motobolt',
            detail: 'Podés elegir encomienda aunque estés en zona Motobolt. Enviamos con Transportadora TSI, Transportadora Jaraha o la de tu preferencia. Después del pago te escribimos para confirmar agencia.'
        });
    }
...
function scanQrOutcome(order) {
    const status = order?.fulfillmentStatus || '';
    const number = order?.orderNumber || 'este pedido';
    if (status === FULFILLMENT.ENTREGADO) {
        return {
            apply: false,
            alreadyDone: true,
            message: `El pedido ${number} ya fue entregado. No se vuelve a marcar.`
        };
    }
    if (status === FULFILLMENT.CANCELADO || status === FULFILLMENT.DEVUELTO) {
        return {
            apply: false,
            alreadyDone: true,
            message: `El pedido ${number} está ${status === FULFILLMENT.DEVUELTO ? 'devuelto' : 'cancelado'}.`
        };
    }
    if ([
        FULFILLMENT.PREPARADO,
        FULFILLMENT.ESPERANDO_MOTOBOLT,
        FULFILLMENT.ENVIANDO,
        FULFILLMENT.ESPERANDO_RETIRO
    ].includes(status)) {
        return {
            apply: false,
            alreadyDone: true,
            message: `El pedido ${number} ya está preparado. No se duplica el estado.`
        };
    }
    return { apply: true, alreadyDone: false };
}
...
function startOfDayInTimeZone(timeZone, date = new Date()) {
    const p = tzParts(date, timeZone);
    const utcMidnightGuess = Date.UTC(p.year, p.month - 1, p.day, 0, 0, 0);
    const offset = timeZoneOffsetMs(timeZone, new Date(utcMidnightGuess));
    let start = new Date(utcMidnightGuess - offset);
    const offset2 = timeZoneOffsetMs(timeZone, start);
    start = new Date(utcMidnightGuess - offset2);
    return start;
}

function paraguayDayBounds(date = new Date()) {
    const start = startOfDayInTimeZone(PY_TZ, date);
    const end = startOfDayInTimeZone(PY_TZ, new Date(start.getTime() + 36 * 60 * 60 * 1000));
    return { start, end };
}

function formatParaguayDate(date = new Date()) {
...
function formatParaguayTime(date = new Date()) {
    return new Intl.DateTimeFormat('es-PY', {
        timeZone: PY_TZ,
        hour: '2-digit',
        minute: '2-digit',
...
function formatParaguayDateTime(date = new Date()) {
    return `${formatParaguayDate(date)} ${formatParaguayTime(date)}`;
}

function parseCajaSaleItems(body = {}) {
    let raw = [];
    if (body.itemsJson) {
        try {
            const parsed = typeof body.itemsJson === 'string' ? JSON.parse(body.itemsJson) : body.itemsJson;
            if (Array.isArray(parsed)) raw = parsed;
        } catch {
            raw = [];
        }
    }
    if (!raw.length && body.productId) {
        raw = [{
    ...
    })).filter((item) => item.productId && item.quantity > 0 && item.sellPrice > 0);
}

function buildCajaSaleDescription(lines) {
    const parts = (lines || []).map((line) => {
        const name = line.variantName ? `${line.name} - ${line.variantName}` : line.name;
        return `${name} (x${line.quantity})`;
    });
    return `Venta: ${parts.join(', ')}`;
}

function applyManualSaleStock(product, qty, variantName) {
    if (!product || !qty) return product;
    if (product.hasVariants && variantName) {
...
module.exports = {
    ...
    parseCajaSaleItems,
    buildCajaSaleDescription,
    applyManualSaleStock,
    whatsappLink,
