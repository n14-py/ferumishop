...
function settingsSchema() {
    return new mongoose.Schema({
        ...
        priorityProductId: { type: String, default: '' },
...
function normalizePriorityProductId(value) {
    const id = String(value || '').trim();
    if (!id || id === 'none' || id === 'cualquiera' || id === '0') return '';
    return id;
}

function fillProductSlots(pool, count) {
    const wanted = clampBatch(count);
    if (!Array.isArray(pool) || !pool.length || wanted <= 0) return [];
    const out = [];
...
function publicSettings(doc) {
    return {
        ...
        priorityProductId: normalizePriorityProductId(doc?.priorityProductId),
...
async function saveSettings(patch = {}) {
    ...
    if (patch.priorityProductId !== undefined) doc.priorityProductId = normalizePriorityProductId(patch.priorityProductId);
    await doc.save();
    const prio = doc.priorityProductId ? `prioridad ${doc.priorityProductId}` : 'prioridad cualquiera';
    await logEvent({
        level: 'info',
        event: 'settings',
        message: `Config: ${doc.enabled ? 'ENCENDIDO' : 'APAGADO'} · ${doc.dailyQuota} videos/día (cuando el bot acepta) · buffer ${BUFFER_SIZE_LIMIT} JSON · ${prio} · borrar a los ${doc.keepDays} días · envío VPS ${doc.autoDispatch ? 'sí' : 'no'}`
    });
    return doc;
}
...
async function pickProducts(Product, { count = TICK_CHUNK, priorityProductId = '' } = {}) {
    const wanted = clampBatch(count);
    const priorityId = normalizePriorityProductId(priorityProductId);

    if (priorityId) {
        const product = await Product.findById(priorityId).populate('category');
        if (!product || !scenes.isEligibleProduct(product)) return [];
        const alive = await verifyProductVideos(product);
        if (!alive.length) return [];
        product.videos = alive;
        return fillProductSlots([product], wanted);
    }
    ...
    return fillProductSlots(pool, wanted);
}
...
async function generateBatch({ Product, SiteConfig, count = TICK_CHUNK, dispatch = false, source = 'manual' }) {
    ...
    try {
        ...
        const products = await pickProducts(Product, {
            count,
            priorityProductId: settings.priorityProductId
        });
        const results = [];
        if (!products.length) {
            const prio = normalizePriorityProductId(settings.priorityProductId);
            await logEvent({
                level: 'warn',
                event: 'no_products',
                message: prio
                    ? 'El producto en prioridad no tiene stock o clips R2 vivos. Elegí otro o dejalo en cualquiera.'
                    : 'No hay productos con stock y video R2 vivo.'
            });
        }
...
async function dashboard({ Product } = {}) {
    ...
    let eligibleCount = 0;
    let priorityProductName = '';
    if (Product) {
        eligibleCount = (await listEligibleProducts(Product, { limit: 200 })).length;
        if (settings.priorityProductId) {
            const prio = await Product.findById(settings.priorityProductId).select('name').lean();
            priorityProductName = prio?.name || '';
        }
    }
    return {
        settings: { ...settings, priorityProductName },
        quota,
        gemma: {
            ok: gemmaCfg.ok,
